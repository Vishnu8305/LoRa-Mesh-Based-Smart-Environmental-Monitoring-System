...
// ---------------- CONFIG ----------------
#define WIFI_SSID "KLEF"
#define WIFI_PASS ""
#define MQTT_SERVER "10.45.10.36"
...
// ===============================
  ...
  int parsed = sscanf(incomingLine,
    "{src:%*d,temp:%f,ph:%f,tds:%f,turb:%f}",
    &temp, &ph, &tds, &turb);

  if (parsed == 4) {
    Serial.println("🌊 Water OK");

    client.publish("water/temperature", String(temp,1).c_str());
    client.publish("water/ph", String(ph,2).c_str());
    client.publish("water/tds", String(tds,0).c_str());
    client.publish("water/turbidity", String(turb,0).c_str());
  } else {
    Serial.println("❌ Water Parse Error");
  }
}
// ===============================
// 🌿 AIR (src:0007)
// ===============================
  ...
  float temp=0, hum=0;
  int co2=0, voc=0, nox=0, pm25=0, pm10=0;

  int parsed = sscanf(incomingLine,
    "{src:%*d,temp:%f,hum:%f,co2:%d,voc:%d,nox:%d,pm25:%d,pm10:%d}",
    &temp, &hum, &co2, &voc, &nox, &pm25, &pm10);

  if (parsed == 7) {
    Serial.println("🌿 Air OK");

    client.publish("air/temperature", String(temp,1).c_str());
    client.publish("air/humidity", String(hum,1).c_str());
    client.publish("air/co2", String(co2).c_str());
    client.publish("air/voc", String(voc).c_str());
    client.publish("air/nox", String(nox).c_str());
    client.publish("air/pm25", String(pm25).c_str());
    client.publish("air/pm10", String(pm10).c_str());

  } else {
    Serial.println("❌ Air Parse Error");
  }
}
